...
% But the library lacked the feature of cache-redirects. Therefore, the fields which were already queried by a list-view, couldn't be reduced when the detail view was queried. The implementation was enhanced by adding a parameter, where a reference to an existing cache object could be specified. If no reference is found in the cache, these references to a cache object are used to reduce the query.
